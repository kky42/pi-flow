...
- Determinism is enforced at parse time via an `acorn` AST scan: `Date.now()`, `Math.random()`, and `new Date()` are rejected. Dynamic authoring, deterministic execution. The scan checks determinism ONLY — it intentionally permits computed member access (`obj[key]`, `arr[i]`, `{ [k]: v }`). It does not attempt vm-escape hardening (no `.constructor`/`Reflect`/`Function`/`this` bans): the `node:vm` is explicitly not a security boundary, and banning dynamic computed access to stop `obj['constructor']` only broke idiomatic model-authored scripts while guarding nothing (the subagents a script spawns already run with full tools).
