...
- Determinism is a cooperative parse-time lint via an `acorn` AST scan: direct `Date.now()`, `Math.random()`, and `new Date()` are rejected for normal model-written scripts. Dynamic authoring, deterministic-by-convention execution. The scan checks determinism ONLY — it intentionally permits computed member access (`obj[key]`, `arr[i]`, `{ [k]: v }`) and does not attempt vm-escape hardening. Do not claim malicious JavaScript is sandboxed.
- `agent()` reuses the shared spawn core, so a `subagent_type` selects a real profile and the subagent gets that profile's model, thinking level, tools, and system prompt — not stubbed guidance. `agent({ schema })` returns a schema-validated object: a terminating `structured_output` tool is injected into the child via `createAgentSession`'s `customTools`, the profile tool allow-list is extended to admit it (by injected tool name), and the first successful call is captured as the result; duplicate successful calls are ignored.
